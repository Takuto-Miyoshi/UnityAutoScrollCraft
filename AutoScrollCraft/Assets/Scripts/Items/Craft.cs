...
namespace AutoScrollCraft.Items {
	public class Craft : MonoBehaviour {
		public struct CraftData {
			List<Enums.Items> materials;  // 素材
			public List<Enums.Items> Materials {
			...
			Enums.Items result;   // 生成物
			public Enums.Items Result {
				get { return result; }
				set { result = value; }
			}
		...
		void Start () {
			if (loaded == false) {
				...
				AddMaterial ( Enums.Items.Stone, 2 );
				SetResult ( Enums.Items.Pork, 5 );
				AddRecipe ();
				AddMaterial ( Enums.Items.Wood, 1 );
				SetResult ( Enums.Items.Stone, 3 );
				AddRecipe ();
				AddMaterial ( Enums.Items.Wood, 1 );
				SetResult ( Enums.Items.Wood, 2 );
				AddRecipe ();
				// ------------------------------------------
		...
		// 下書きに素材を追加
		static void AddMaterial ( Enums.Items item, int value ) {
			tmpRecipe.Materials.Add ( item );
			tmpRecipe.MaterialAmountList.Add ( value );
		}

		// 下書きの生成物を設定
		static void SetResult ( Enums.Items item, int value ) {
			tmpRecipe.Result = item;
			tmpRecipe.ResultAmount = value;
		}

		// アイテムを作れるか
		static public bool CanBeCrafting ( Actors.Player.ItemData[] inventory, int recipeNum ) {
